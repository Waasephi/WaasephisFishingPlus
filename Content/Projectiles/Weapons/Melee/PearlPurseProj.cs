...
namespace WaasephisFishingPlus.Content.Projectiles.Weapons.Melee
{
	...
	public class PearlPurseProj : ModProjectile
	{
		private const string ChainTexturePath = "WaasephisFishingPlus/Content/Projectiles/Weapons/Melee/PearlPurseChain"; // The folder path to the flail chain sprite
		...
		public override void AI()
		{
			...
			float maxRetractSpeed = 12f; // The max speed the projectile will have while retracting
			...
			float unusedRetractAcceleration = 8f;
				...
				case AIState.LaunchingForward:
					{
						...
						if (player.controlUseItem) // If the player clicks, transition to the Dropping state
						{
							...
							if (Main.myPlayer == Projectile.owner)
								Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), Projectile.Center, Projectile.velocity * 0.5f, ModContent.ProjectileType<WhitePearlProj>(), Projectile.damage, Projectile.knockBack, Main.myPlayer);
			...
			*/
			...
			if (Main.rand.NextBool(dustRate))
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.TreasureSparkle, 0f, 0f, 150, default(Color), 1.3f);
		}
